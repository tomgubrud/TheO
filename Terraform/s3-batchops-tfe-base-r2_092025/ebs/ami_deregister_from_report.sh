...
#
# Typical flow:
#   1) Run list_ami_snapshot_usage.sh (with cutoff year), review CSV.
...
#
# Notes:
#   - This script DOES NOT delete snapshots. It only deregisters AMIs.
#   - Includes a dry-run mode that only prints commands.
#   - Performs basic safety checks to see if AMI is referenced by:
#       * Launch Templates (any version)
#       * Auto Scaling Launch Configurations
#     If references are found, that AMI is SKIPPED unless you pass --force.
#
# Requirements:
#   - Bash, AWS CLI v2, coreutils (awk/grep/sed/sort/uniq), no jq needed.
#
# Usage:
...
# ===== Flags =====
FORCE_NOREFS="N"   # If Y, will deregister even if references found
...
# Skip header; pick lines with YES in the 7th column; capture Region and AMIId
awk -F',' 'NR>1 && toupper($7) ~ /YES/ && $4 != "" {gsub(/^[ \\t]+|[ \\t]+$/,"",$1); gsub(/^[ \\t]+|[ \\t]+$/,"",$4); print $1" "$4}' "$CSV"   | sort -u > "$CANDIDATES"
...
check_references() {
  ...
  local lt_ids
  lt_ids=$(aws ec2 describe-launch-templates --region "$region"              --query "LaunchTemplates[].LaunchTemplateId" --output text 2>>"$LOG_FILE" | tr '\t' '\n' || true)
  if [[ -n "${lt_ids:-}" ]]; then
    while read -r lt; do
      [[ -z "$lt" ]] && continue
      if aws ec2 describe-launch-template-versions --region "$region" --launch-template-id "$lt"            --versions All --output text 2>>"$LOG_FILE" | grep -qw "$ami"; then
        echo "LT:$lt"
        return 0
      fi
    done <<< "$lt_ids"
  ...
  local lc_match
  lc_match=$(aws autoscaling describe-launch-configurations --region "$region"                --query "LaunchConfigurations[?ImageId=='$ami'].LaunchConfigurationName"                --output text 2>>"$LOG_FILE" || true)
  if [[ -n "${lc_match:-}" ]]; then
    echo "LC:$lc_match"
    return 0
  fi
...
while read -r REGION AMI_ID; do
  ...
  AMI_STATE=$(aws ec2 describe-images --region "$REGION" --image-ids "$AMI_ID"                --query "Images[0].State" --output text 2>>"$LOG_FILE" || echo "deregistered")
...
echo "Candidates: $TOTAL | Deregistered: $SUCC | Skipped: $SKIP | Failures: $FAIL" | tee -a "$LOG_FILE"
