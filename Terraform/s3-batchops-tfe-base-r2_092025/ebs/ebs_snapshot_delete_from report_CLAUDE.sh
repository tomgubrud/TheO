...
# Determine if we're using region filtering
USE_REGION_FILTER="Y"
...
# Fixed AWK script with corrected region filtering logic
awk -F',' -v region_filter="$REGION_FILTER" -v use_filter="$USE_REGION_FILTER" '
BEGIN {
  # Build allowed regions map if filtering
  if (use_filter == "Y" && length(region_filter) > 0) {
...
NR > 1 {
  ...
  # Only process rows marked safe to deregister
  if (toupper(safe) != "YES")
    next
  
  # Apply region filter if enabled
  if (use_filter == "Y" && !(reg in allowed))
    next
...
log "[$(date)] Candidates after filtering: $TOTAL"

# If still getting zero, let's debug the CSV
if [[ "$TOTAL" -eq 0 ]]; then
  log "[$(date)] WARNING: No candidates found after filtering!"
  log "[$(date)] Debugging CSV structure..."
  
  # Show first few lines of CSV for debugging
  log "[$(date)] First 3 lines of CSV:"
  head -n 3 "$CSV_PATH" | while IFS= read -r line; do
    log "  $line"
  done
  
  # Show unique regions in CSV
  log "[$(date)] Unique regions found in CSV:"
  awk -F',' 'NR>1 {gsub(/^[ \t]+|[ \t]+$|"/,"",$1); if ($1 != "") print $1}' "$CSV_PATH" | sort -u | while read -r r; do
    log "  - $r"
  done
  
  # Show how many rows have SafeToDeregister=YES
  log "[$(date)] Rows with SafeToDeregister=YES by region:"
  awk -F',' 'NR>1 && toupper($7) ~ /YES/ {gsub(/^[ \t]+|[ \t]+$|"/,"",$1); if ($1 != "") print $1}' "$CSV_PATH" | sort | uniq -c | while read -r count reg; do
    log "  - $reg: $count"
  done
