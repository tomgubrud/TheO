...
# --- replace your old human_bytes() with this pure-bash version ---
human_bytes() {
  # prints e.g. "3.42 GiB" using integer math, no awk/bc/jq
  local b="${1:-0}" units=(B KiB MiB GiB TiB PiB) i=0
  # guard non-numeric
  [[ "$b" =~ ^[0-9]+$ ]] || b=0
  while (( b >= 1024 && i < ${#units[@]}-1 )); do
    # scale by 100 to keep two decimals without floats
    b=$(( (b * 100 + 512) / 1024 ))
    (( i++ ))
  done
  if (( i == 0 )); then
...
[[ "$TOTAL_SELECTED_OBJS" =~ ^[0-9]+$ ]] || TOTAL_SELECTED_OBJS=0
[[ "$TOTAL_SELECTED_BYTES" =~ ^[0-9]+$ ]] || TOTAL_SELECTED_BYTES=0

hb_total="$(human_bytes "$TOTAL_SELECTED_BYTES")"
printf " Selected P1 total (SRC peek): %12d  (%s)\n" \
       "$TOTAL_SELECTED_OBJS" "$hb_total"
echo
